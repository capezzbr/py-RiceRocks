...
class Sprite:
    ...
    def update(self):
        ...
        # update the age and return true if the age has exceeded the lifespan
        self.age += 1
        return self.age > self.lifespan
        
    def collide(self, other_object):
        distance = dist(self.pos, other_object.get_position())
        hasCollision = distance < self.radius + other_object.get_radius()
        return hasCollision
...
def draw(canvas):
    ...
    # update collisions between missiles and rocks, also update the points
...
def rock_spawner():
    ...
    # ignore this rock if is spawn too close to my_ship
    distance = dist(my_ship.get_position(), rock_pos)
    tooClose = distance < my_ship.get_radius() + asteroid_info.get_radius() + SAFETY_DISTANCE
    if not tooClose:
        rock_group.add(Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info))
    
# call the update / draw method for every object inside a group
def process_sprite_group(canvas, group):
    ...
    for sprite in group:
        if sprite.update():
            to_remove.add(sprite) # has eceeded his lifespan time
            
    # cleanup
    group.difference_update(to_remove)            
    
# return true if there is a collision between other_object sprite and one of the group's element
...
# check for a collision between two groups
def group_group_collide(a_group, another_group):
    collides = set()
    for element in a_group:
        if group_collide(another_group, element):
            collides.add(element)
            break
            
    a_group.difference_update(collides)
    return len(collides)
        
# called when the lives are < 1 
def game_over():
